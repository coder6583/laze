...
{
    T_exp addr = T_BinOpExp(T_i32, T_add, T_BinOpExp(T_i32, T_mul, T_BinOpExp(T_i32, T_mul, T_GetLocalExp(T_i32, 0), T_ConstExp(T_i32, A_IntExp(0, 4))), T_ConstExp(T_i32, A_IntExp(0, 512))), T_BinOpExp(T_i32, T_mul, T_GetLocalExp(T_i32, 1), T_ConstExp(T_i32, A_IntExp(0, 4))));
    T_exp color = T_GetLocalExp(T_i32, 2);
...
struct expty transVar(S_table venv, S_table tenv, A_var v, Tr_level level)
{
    switch(v -> kind)
    {
        case A_simpleVar:
        {
            ...
            {
                return expTy(Tr_VarExp(v -> pos, convertType(x -> u.var.ty), x -> u.var.access), actual_ty(x -> u.var.ty));
            }
            else if(!x)
            {
    ...
    switch(e -> kind)
    {
        ...
        case A_callExp:
        {
            ...
            if(entry != NULL && entry -> kind == E_funcentry)
            {
                ...
                return expTy(Tr_CallStm(e -> pos, entry -> u.func.index, S_name(entry -> u.func.label), result), entry -> u.func.result);
            }
            else
            {
